...
C. **每個數字都要對得回一張來源表**：報告裡每一處「分子/分母」都必須在 B 的索引裡
   ...
   ⚠️ C 擋得住的與擋不住的，要分清楚：
     ...
     · **擋不住**：分子分母確實存在於某張表，但那張表回答的不是這句話要講的事。
       ...
       這一類只能靠人看，紀錄在 04_需求分析與選題報告/取數來源清查.md。
...
# 好讀版（2026-07-27 起）也列入：它拆掉了大部分括號分數，這道關卡能查的比例變少，
# 但條件 11 要求的「N 人裡有 M 人」小樣本寫法仍會被抓進來重算，且分子分母仍須對得回
# 99_數字索引.csv——不是新寫檢查，只是讓好讀版受同一套既有機制保護（好讀版站①條件 40）。
REPORTS = [
    C.OUT_DIR / "公民科技生態系分析報告_v1.md",
    C.OUT_DIR / "公民科技生態系分析報告_整合版_v1.md",
    C.OUT_DIR / "公民科技生態系分析報告_好讀版_v1.md",
]
