...
# 讀法說明會原封不動寫進 CSV，外部讀者看得到。
# 三個原則：① 先講能怎麼讀、再講不能怎麼讀（連續三個「不可」會讀起來像自我否認）；
#          ② 不用程式變數名與分析術語（ROLE_LADDER、分母池）；③ 每段開頭先交代這張表算的是誰。
ROLE_NOTE = ("這張表算的是做過專案的 47 人。"
             "可以這樣讀：五個角色各自的水準。"
             "不要這樣讀——"
             "（1）表中五列的排列順序沒有含意，只是固定的呈現順序，不代表角色由淺到深："
             "勾「出錢或調度資源」的人完全可能沒使用過、也沒推廣過任何專案；"
             "（2）角色是複選，同一個人可以同時落在好幾列，所以各列不是互斥的類別，不能互相比高低；"
             "（3）各群的「該群平均角色廣度」不一樣（3.49–4.56），"
             "群與群之間的差異可能只是「身兼多職的人撞到比較多種困難」。")

BREADTH_NOTE = ("這張表算的是做過專案的 47 人。"
                "角色廣度（一個人勾了幾種角色，1–5）是互斥且有序的分組，可以讀成順序。"
                "「切法」欄區分兩套切法：二分組與逐級。"
                "**兩套不可混在一起加總**——同一批 47 人被切了兩次，全部相加會得到 94。"
                "逐級各只有 5–11 人，僅供追溯。")

DEPTH_NOTE = ("這張表算的是做過專案的 47 人。"
              "參與深度是單選、各級互斥，且由低到高有序（依主動權多寡），可以讀成順序。"
              "但各級只有 5–18 人，**請以中位數為主指標**——"
              "平均容易被單一個案拉動，例如「接收資訊」組平均 3.00 但中位數只有 1.0，"
              "是某一人在 13 題中有 10 題達標造成的。")

MULTI_NOTE = ("各列分子＝勾了該選項的人數，**跨選項不可相加**"
              "（同一個人可能勾了多項，相加會重複計同一個人）。"
              "選項以外的自由填答已合併成「其他（自由填答）」。"
              "勾選人數少於 5 人的列不提供倍數（lift），因為那個倍數會被一兩個人左右。")
...
def main():
    ...
    OVERLAP_NOTE = ("這張表算的是接觸過公民科技的 101 人。"
                    "讀法：分母是勾了 A 的人數，比例是「這些人當中也勾了 B 的比例」。"
                    "**方向不可反讀**——使用者→出資者 0.1889 的意思是「勾使用者的人有 19% 也勾出資者」，"
                    "不是「19% 的出資者是使用者」（後者是 100%，就在下面那幾列）。"
                    "另外，五個角色**不是層層包含的階梯**：勾發起者的人只有 82% 也勾使用者，"
                    "所以分母從 90 遞減到 17 不可讀成一個漏斗。")
    ov_rows = []
    ...
    sp = pd.DataFrame([{
        ...
        "讀法說明": ("這一列算的是做過專案的 47 人。相關不等於因果，本列不得用於任何因果宣稱；"
    ...
    CHECK_NOTE = ("用途：核對 10_*.csv 的每一個數字。"
                  "這張表涵蓋接觸過公民科技的 101 人，但**專案層級的題目只有做過專案的 47 人被問過**——"
                  "困擾／資源／動機欄位若是空白，表示這個人沒被問，**不是 0**"
                  "（若當 0 計入，使用者組的達3題數平均會從 2.41 掉到 1.04）。"
                  "「填答序號」是去識別化資料的列序（0 起算、有跳號），不是 Excel 的列號。")
    check = pd.DataFrame({
        "入坑分層": df[C.LAYER],
        "角色廣度": breadth,
        "參與深度": idx["參與深度"],
